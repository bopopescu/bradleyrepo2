...
def addons():
    ...
    resp = dialog.select('[COLOR fffea800]iVue Default Shortcuts[/COLOR]', ['BBC iPlayer', 'ITV player', 'FTV filmon', 'Todays sports', 'Exodus', 'Specto', 'Youtube', 'Phoenix', 'Supremacy', 'Match center'])
    if resp == 0:
        title = 'BBC iPlayer'
    ...
    if resp == 2:
        title = 'FTV filmon'
        image = 'special://home/addons/script.ivueguide/resources/png/FTV icon.png'
        link = 'RunAddon(plugin.video.F.T.V)'
        filter.append(title)
        filter.append(image)
        filter.append(link)
    if resp == 3:
        title = 'Todays sport'
        image = 'special://home/addons/script.ivueguide/resources/png/ivuesports.jpg'
        link = 'RunAddon(plugin.video.)'
        filter.append(title)
        filter.append(image)
        filter.append(link)
    if resp == 4:
        title = 'Exodus'
        image = 'special://home/addons/script.ivueguide/resources/png/exodus icon.png'
        link = 'RunAddon(plugin.video.exodus)'
        filter.append(title)
        filter.append(image)
        filter.append(link)
    if resp == 5:
    ...
    if resp == 7:
        title = 'Phoenix'
        image = 'special://home/addons/script.ivueguide/resources/png/ph icon.png'
        link = 'RunAddon(plugin.video.phstreams)'
        filter.append(title)
        filter.append(image)
        filter.append(link)
    if resp == 8:
    ...
    if resp == 9:
        title = 'Match center'
        image = 'special://home/addons/script.ivueguide/resources/png/match icon.png'
        link = 'RunScript(script.matchcenter)'
        filter.append(title)
        filter.append(image)
        filter.append(link)
    return filter